        ...
        while not flag:
            try:
                ...
                response = client.chat.completions.create(
                    model=MODEL_TYPE,
                    messages=message,
                    max_tokens=256,  # 🌟 加上这行，给模型足够的生成长度额度
                    temperature=0.0  # (可选) 设为 0 可以保证输出更稳定的确定性结果
                )
...
def val():
    ...
    for i in range(2):
        ...
        # --- 加上这两行自动创建目录 ---
        os.makedirs(os.path.dirname(result_save_path1), exist_ok=True)
        os.makedirs(os.path.dirname(actions_save_path1), exist_ok=True)
        # ------------------------------
...
def test():
    ...
    for i in range(3):
        ...
        total_result.append(result)
        os.makedirs(os.path.dirname(result_save_path2), exist_ok=True)
        os.makedirs(os.path.dirname(actions_save_path2), exist_ok=True)
